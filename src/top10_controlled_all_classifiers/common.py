...
def load_data():
    ...
    missing_classifier_cols = [col for col in CLASSIFIER_COLS if col not in performance_matrix.columns]
    if missing_classifier_cols:
        raise RuntimeError(
            "Classificadores alvo ausentes na matriz de performance: "
            + ", ".join(missing_classifier_cols)
        )

    for col in existing_classifier_cols:
        performance_matrix[col] = pd.to_numeric(performance_matrix[col], errors="coerce")

    if REQUIRE_ALL_CLASSIFIER_COLS:
        performance_matrix = performance_matrix.dropna(subset=existing_classifier_cols).copy()
    else:
        performance_matrix = performance_matrix.dropna(subset=existing_classifier_cols, how="all").copy()

    if performance_matrix.empty:
        raise RuntimeError(
            "Nenhum dataset possui resultados suficientes para os classificadores alvo: "
            + ", ".join(existing_classifier_cols)
        )
...
def run_lodo_experiment(rank, strategy, semantic_mode, domain_mode, output_filename):
    """
    Executes a Leave-One-Domain-Out (LODO) cross-validation experiment.
    """
    # 1. Load the data using your existing function
    X, y, numeric_cols, classifier_cols = load_data()
    encoder = LabelEncoder()
    y_encoded = encoder.fit_transform(y)
    
    # 2. Extract the groups (domains) for the LODO split
    # This tells scikit-learn which rows belong to which domain
    groups = X["predicted_domain"].astype(str).to_numpy()
    
    # 3. Instantiate the Leave-One-Group-Out cross-validator
    ...
    # CRITICAL: You must pass the 'groups' array here!
    ...
    # In LODO, the number of splits is exactly the number of unique domains
    n_splits = len(np.unique(groups))
    
    # 6. Format and save the results
    summary = pd.DataFrame([{
        "rank": rank,
        "strategy": strategy + "_LODO", # Appending _LODO to differentiate results
    ...
    print(f"--- LODO Evaluation: {strategy} ---")
    print(f"Tested across {n_splits} distinct domains.")
