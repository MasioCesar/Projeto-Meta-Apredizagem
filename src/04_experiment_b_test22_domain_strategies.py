...
class DomainConfidenceFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        scores = pd.to_numeric(pd.Series(np.asarray(X).ravel()), errors="coerce").fillna(0)
...
class DomainSemanticInteraction(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        df = pd.DataFrame(X, columns=["semantic_text", "name", "predicted_domain", "domain_score"])
        tag_extractor = SemanticTagExtractor()
        tag_values = tag_extractor.transform(df[["semantic_text", "name"]])
        domains = df["predicted_domain"].fillna("unknown").astype(str)
        scores = pd.to_numeric(df["domain_score"], errors="coerce").fillna(0).to_numpy(dtype=float)
        score_scale = np.log1p(scores).reshape(-1, 1)

        features = []
        for domain in self.domains_:
            mask = (domains == domain).to_numpy(dtype=float).reshape(-1, 1)
...
def load_data():
    ...
    ignore_cols = [
        "did", "domain", "best_classifier", "best_accuracy",
    ] + all_possible_classifier_cols
...
def controlled_semantic_transformer():
    return FeatureUnion([
        ("tags", Pipeline([
...
def build_model(numeric_cols, use_semantic, domain_mode):
    transformers = [
        (
            "statistical",
            Pipeline([
    ...
    if use_semantic:
        transformers.append((
            "controlled_semantic",
            controlled_semantic_transformer(),
            ["semantic_text", "name"],
        ))
...
def main():
    ...
    experiments = [
        ("stat_only_no_domain_score", False, "none"),
        ("stat_plus_domain_label", False, "label"),
        ("stat_plus_domain_score", False, "score"),
        ("stat_plus_domain_label_score", False, "label_score"),
        ("stat_plus_semantic_no_domain_score", True, "none"),
        ("stat_plus_semantic_domain_label", True, "label"),
        ("stat_plus_semantic_domain_score", True, "score"),
        ("stat_plus_semantic_domain_label_score", True, "label_score"),
        ("stat_plus_semantic_domain_interaction", True, "label_score_interaction"),
    ]

    print("\nEXPERIMENTO B - TESTE 22: ESTRATEGIAS COM DOMINIO")
    print("Importante: domain_score foi removido da baseline estatistica e so entra nas estrategias de dominio.")
    print("-" * 102)
    print(f"{'estrategia':<46} | {'semantica':<9} | {'dominio':<23} | {'acc':>8} | {'f1_macro':>8}")
    print("-" * 102)

    summaries = []

    for strategy, use_semantic, domain_mode in experiments:
        model = build_model(numeric_cols, use_semantic, domain_mode)
        ...
        row = {
            "experiment": "B_Test22_Domain_Strategies",
            "strategy": strategy,
            "uses_controlled_semantics": use_semantic,
            "domain_mode": domain_mode,
            "features": "Stat"
            + (" + Controlled Semantic" if use_semantic else "")
            + ("" if domain_mode == "none" else f" + Domain({domain_mode})"),
    ...
    print(
        final_df[
            ["strategy", "uses_controlled_semantics", "domain_mode", "accuracy_mean", "f1_macro_mean"]
