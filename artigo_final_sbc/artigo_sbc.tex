...
algorithm within it (a target encoding fitted per training fold) improves both
primary metrics: $+1.19$ accuracy and $+0.69$ F1-macro percentage points (both
p $<$ 0.001 over 30 seeds, positive in 24/30) under leakage-controlled grouped
cross-validation. Naive encodings---one-hot, keyword score, TF-IDF, dense
embeddings, learned semantic clusters---show no effect. We reach this trustworthy
...
(um \textit{target encoding} ajustado por fold de treino), melhoram-se as duas
métricas principais: $+1{,}19$ p.p.\ de acurácia e $+0{,}69$ p.p.\ de F1-macro
(ambas p $<$ 0,001 em 30 sementes, positivas em 24/30) sob validação cruzada
agrupada e livre de vazamento. Codificações ingênuas---one-hot, \textit{score}
por palavras, TF-IDF, embeddings densos, clusters semânticos aprendidos---não
mostram efeito. Chegamos a esse resultado confiável apenas após descartar os
...
rank médio, melhora a seleção (acurácia $+1{,}19$ p.p.; F1-macro $+0{,}69$ p.p.;
p $<$ 0,001) em 547 datasets sem viés, sob validação livre de vazamento; (ii)
evidência de que o alvo é dominado por ruído (26\% de rótulos de semente única
instáveis; margens $\sim$0,8 p.p.\ entre algoritmos), o que limita o ganho
possível; (iii) um protocolo metodológico para separar sinal de artefato; e (iv)
...
\begin{table}[H]
\centering
\caption{Ganho do domínio (\textit{target encoding} por rank médio), CV agrupada,
30 sementes. Significativo e replicável em acurácia e F1-macro.}
\label{tab:principal}
\begin{tabular}{lccc}
\toprule
Métrica & Baseline & + domínio (TE) & Significância \\
\midrule
...
o sinal é real e pequeno, e aparece sob codificação supervisionada por rank médio,
com validação rigorosa e livre de vazamento (acurácia $+1{,}19$ p.p.\ e F1-macro
$+0{,}69$ p.p., ambas p $<$ 0,001 em 30 sementes). Chegamos a ele descartando
sistematicamente os artefatos que enganariam uma análise ingênua, o que sustenta a
confiabilidade da conclusão. Como contribuição transferível, recomendamos
